...
async function dummyData(tx) {
    ...
    // Create a new Farmer.
    var farmer = factory.newResource('org.example.biznet', 'Farmer', 'FARMER_001');
    ...
    var shipper = factory.newResource('org.example.biznet', 'Shipper', 'SHIPPER_001');
    // Set the properties of the new farmer.
    ...
    var warehouseman = factory.newResource('org.example.biznet', 'Warehouseman', 'WAREHOUSEMAN_001');
    // Set the properties of the new farmer.
    ...
    var distributor = factory.newResource('org.example.biznet', 'Distributor', 'DISTRIBUTOR_001');
    // Set the properties of the new farmer.
    ...
    var retailer = factory.newResource('org.example.biznet', 'Retailer', 'RETAILER_001');
    // Set the properties of the new farmer.
    ...
    // Product
    var product = factory.newResource('org.example.biznet', 'Product', 'PROD_001');
    ...
    product.rating = 4.5;
    product.owner = factory.newRelationship('org.example.biznet', 'Farmer', 'FARMER_001');
    // var newTrace = factory.newConcept('org.example.biznet', 'Trace');
    ...
    // Contract1
    var contract1 = factory.newResource('org.example.biznet', 'Contract', 'CON_001');
    // Set the properties of the new vehicle.
    contract1.farmer = factory.newRelationship('org.example.biznet', 'Farmer', 'FARMER_001');
    contract1.distributor = factory.newRelationship('org.example.biznet', 'Distributor', 'DISTRIBUTOR_001');
    // contract1.retailer = factory.newRelationship('org.example.biznet', 'Retailer', 'RETAILER_001');
    contract1.product = factory.newRelationship('org.example.biznet', 'Product', 'PROD_001');
    ...
    // Contract2
    var contract2 = factory.newResource('org.example.biznet', 'Contract', 'CON_002');
    // Set the properties of the new vehicle.
    // contract2.farmer = factory.newRelationship('org.example.biznet', 'Farmer', 'FARMER_001');
    contract2.distributor = factory.newRelationship('org.example.biznet', 'Distributor', 'DISTRIBUTOR_001');
    contract2.retailer = factory.newRelationship('org.example.biznet', 'Retailer', 'RETAILER_001');
    contract2.product = factory.newRelationship('org.example.biznet', 'Product', 'PROD_001');
    ...
    // Shipment1
    var shipment1 = factory.newResource('org.example.biznet', 'Shipment', 'SHIP_001');
    // Set the properties of the new vehicle.
    shipment1.contract = factory.newRelationship('org.example.biznet', 'Contract', 'CON_001');
    shipment1.modeOfTransport = 'Truck';
    shipment1.currentLocation = 'Thrissur';
    shipment1.shipmentStatus = 'CREATED';
    shipment1.inCustodyOf = factory.newRelationship('org.example.biznet', 'Farmer', 'FARMER_001');

    // Shipment2
    var shipment2 = factory.newResource('org.example.biznet', 'Shipment', 'SHIP_002');
    // Set the properties of the new vehicle.
    shipment2.contract = factory.newRelationship('org.example.biznet', 'Contract', 'CON_002');
    shipment2.modeOfTransport = 'Truck';
    shipment2.currentLocation = 'Thrissur';
    shipment2.shipmentStatus = 'CREATED';
    shipment2.inCustodyOf = factory.newRelationship('org.example.biznet', 'Distributor', 'DISTRIBUTOR_001');


    return getParticipantRegistry('org.example.biznet.Farmer')
        .then(function (farmerRegistry) {
            // add the growers
            return farmerRegistry.addAll([farmer]);
        })
        .then(function () {
            return getParticipantRegistry('org.example.biznet.Shipper');
        })
        ...
        .then(function () {
            return getParticipantRegistry('org.example.biznet.Warehouseman');
        })
        ...
        .then(function () {
            return getParticipantRegistry('org.example.biznet.Distributor');
        })
        ...
        .then(function () {
            return getParticipantRegistry('org.example.biznet.Retailer');
        })
        ...
        .then(function () {
            return getAssetRegistry('org.example.biznet.Contract');
        })
 ...
 * @param {org.example.biznet.CreateProduct} tx
 * @transaction
 */
async function createProduct(tx) {
    var factory = getFactory();
    var product = factory.newResource('org.example.biznet', 'Product', 'PROD_101');
    ...
    product.owner = factory.newRelationship('org.example.biznet', tx.ownerType, tx.ownerId);
    // product.inCustodyOf = factory.newRelationship('org.example.biznet', currentParticipantType, currentParticipantIdentifier);
 ...
 * @param {org.example.biznet.CreateContract} tx
 * @transaction
 */
async function createContract(tx) {
    var factory = getFactory();
    var contract = factory.newResource('org.example.biznet', 'Contract', 'CON_101');

    //Participants in contract
    // if (tx.manufacturerId !== 'null'){
    //     contract.manufacturer = factory.newRelationship('org.example.biznet', 'Manufacturer', tx.manufacturerId);
    // }
    if (tx.warehousemanId !== 'null') {
        contract.warehouseman = factory.newRelationship('org.example.biznet', 'Warehouseman', tx.warehousemanId);
    }
    if (tx.farmerId !== 'null') {
        contract.farmer = factory.newRelationship('org.example.biznet', 'Farmer', tx.farmerId);
    }
    if (tx.distributorId !== 'null') {
        contract.distributor = factory.newRelationship('org.example.biznet', 'Distributor', tx.distributorId);
    }
    if (tx.retailerId !== 'null') {
        contract.retailer = factory.newRelationship('org.example.biznet', 'Retailer', tx.retailerId);
    }
    if (tx.shipperId !== 'null') {
        contract.shipper = factory.newRelationship('org.example.biznet', 'Shipper', tx.shipperId);
    }
    // contract.input = factory.newRelationship('org.example.biznet', 'Product', tx.inputProductId);
    contract.product = factory.newRelationship('org.example.biznet', 'Product', tx.productId);
 ...
 * @param {org.example.biznet.CreateShipment} tx
 * @transaction
 */
async function createShipment(tx) {
    var factory = getFactory();
    var shipment = factory.newResource('org.example.biznet', 'Shipment', 'Ship_101');
    shipment.contract = factory.newRelationship('org.example.biznet', 'Contract', tx.contractId);
    ...
    // shipment.owner = factory.newRelationship('org.example.biznet', 'Contract', tx.ownerId);
    // shipment.inCustodyOf = factory.newRelationship('org.example.biznet', 'Contract', tx.inCustodyOfId);
    // Get the current participant.
    // var currentParticipant = getCurrentParticipant();
    // var currentParticipantType = currentParticipant.getType();
    // var currentParticipantIdentifier = currentParticipant.getIdentifier();

    // // shipment.owner = factory.newRelationship('org.example.biznet', currentParticipantType, currentParticipantIdentifier);
    shipment.inCustodyOf = factory.newRelationship('org.example.biznet', tx.inCustodyOfType, tx.inCustodyOfId);
 ...
 * @param {org.example.biznet.ProductOwnerChange} tx
 * @transaction
 */
async function productOwnerChange(tx) {
    ...
    tx.product.owner = factory.newRelationship('org.example.biznet', tx.ownerType, tx.ownerId);
    const productRegistry = await getAssetRegistry('org.example.biznet.Product');
    await productRegistry.update(tx.product);
}
 ...
 * @param {org.example.biznet.ShipmentInCustodyOfChange} tx
 * @transaction
 */
async function shipmentInCustodyOfChange(tx) {
    ...
    tx.shipment.inCustodyOf = factory.newRelationship('org.example.biznet', tx.inCustodyOfType, tx.inCustodyOfId);
    const shipmentRegistry = await getAssetRegistry('org.example.biznet.Shipment');
    await shipmentRegistry.update(tx.shipment);
}
